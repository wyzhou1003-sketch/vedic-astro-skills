...
时间精度: ±5分钟（rectifier校准完成 2026-06-12）
时间来源: 家人大概回忆 → 经7事件Dasha匹配+扫描定位校准
有效精度: ±5分钟（23:15-23:25，中点23:20）
时间可信度: 高（经vedic-rectifier）
...
| Moon | Rohini | 3 | Moon |
| Mars | Chitra | 1 | Mars |
| Mercury | Purva Phalguni | 3 | Venus |
| Jupiter | Anuradha | 3 | Saturn |
| Venus | Magha | 1 | Ketu |
| Saturn | Purva Bhadrapada | 3 | Jupiter |
| Rahu | Chitra | 4 | Mars |
| Ketu | Ashwini | 2 | Ketu |
...
| Jupiter | Scorpio | Mars | great_friend | |
| Venus | Leo | Sun | great_enemy | |
...
| 1 | Moon旺2宫 → 母亲照顾型 | ❌未命中 |
| 2 | 3宫SAV全盘最高 → 有手足 | ✅命中 |
| 3 | Rahu-Venus小运 → 恋情开始窗口 | ❌未命中 |
| 4 | Rahu-Sun小运 → 成果/认可 | ✅命中 |
| 5 | Jup-Jup小运8宫 → 阶段中断/转换 | ✅命中 |

## 生时矫正记录

```
轨道3双Lagna对比：金牛1°18'(23:30) vs 白羊(±15分钟窗口)
对比结果：白羊上升胜出（2:0有效区分项）
rectifier校准（2026-06-12）：7个人生事件Dasha匹配7/7 → Lagna=白羊确认
时间扫描：Lagna边界23:25，D9射手区间23:14-23:25，与记忆窗口交集23:15-23:25
最终结论：23:20 ±5分钟（情况A：与工作时间一致，无需重算）
Nakshatra校验：Krittika/Bharani未区分（用户回复"都有点"），保持D9定位
方法极限：事件为月份精度，±5分钟为当前极限；窗口内D9/D10/D4/D5恒定
```
...
| 轮次 | AI预测 | 信号调整 |
|------|--------|---------|
| R1 | Moon旺2宫→母亲照顾型 | 未命中；Moon/4宫的母亲软性信号降权 |
| R1 | 3宫SAV最高→有手足 | 命中；SAV结构信号可靠 |
| R1 | Rahu-Venus→恋情窗口 | 未命中；Venus燃烧+大敌的欺骗性强于预期，感情类时间锚点待校准 |
| R1 | Rahu-Sun→成果认可 | 命中；Sun强星时间信号可靠 |
| R1 | Jup-Jup 8宫→阶段中断 | 命中；8宫突变信号可靠 |

解读总结（给core）：
"此盘强星(Sun/Saturn)驱动的事件信号可靠，Dasha时间轴基本成立；
Venus(燃烧+大敌)和Moon相关的感情/家庭软性信号方向易失准，分析时降权处理。"

## 分盘启用标注（按有效精度±15分钟）

```
D1  ✅ 完全启用
D9  ✅ 完全启用（23:15-23:25全窗口D9=射手恒定）
D10 ✅ 完全启用（全窗口D10=摩羯恒定，且事业轨迹交叉验证通过）
D5  ✅ 完全启用（全窗口恒定）
D4  ✅ 完全启用（全窗口恒定）
依据：rectifier校准至±5分钟；校准窗口内四分盘上升唯一确定，
分盘结构不受残余时间误差影响。
```

## 盘外验证（rectifier Step 5）

```
模式A盲审：可计分2条，命中1.5（75%）→ ✅ 通过
亮点：D4分盘结构性推断直接命中 → D4可信度获得实证支撑
1条因盘主无法回忆而不计分
信号调整：L2燃烧的"财库受损"信号方向正确但程度需留意（部分准）
```
